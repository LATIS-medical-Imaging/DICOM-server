...
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" --profile "$PROFILE" run --rm \
  -e K6_OUT="json=/results/load/k6_load_${TIMESTAMP}.json" \
  k6 run /scripts/load_test.js \
  --summary-export="/results/load/k6_summary_${TIMESTAMP}.json" || true
